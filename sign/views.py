...
def signin(request):
    ...
    query2 = "select * from sign_appointment where id='" + str(data[0]) + "'"
    cursor.execute(query2)
    data1 = cursor.fetchone()
    if data[5]==psw:
        data= {'User_Id':data[0],'Name':data[1],'Contact_number':data[2],'Address':data[3],'Email':data[4],'Password':data[5],'appointment_id':data1[0],'User_id':data1[4],'timedate':data1[1],'worker_id':data[4]}
